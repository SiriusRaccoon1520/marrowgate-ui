...
import { THEMES, MODELS, BUBBLE_STYLES, fetchModels, getWorldBook, saveWorldBook, matchWorldBook } from '../lib/config.js';
...
export function useChatState() {
  ...
  const [bubbleStyle, setBubbleStyle] = useState('solid');
  ...
  const [customColors, setCustomColors] = useState(() => {
    ...
    try {
      const saved = localStorage.getItem('mg_custom_colors');
  ...
  const [gatewayUrl, setGatewayUrl] = useState(() => {
    if (typeof window === 'undefined') return '';
    return localStorage.getItem('mg_gateway_url') || '';
  });
  ...
  // 世界书条目
  const [worldBookEntries, setWorldBookEntries] = useState(() => {
    if (typeof window === 'undefined') return [];
    return getWorldBook();
  });
  ...
  const [mcpServers, setMcpServers] = useState(() => {
    if (typeof window === 'undefined') return [];
    try {
      return JSON.parse(localStorage.getItem('mg_mcp_servers') || '[]');
    } catch { return []; }
  });
  ...
  // 持久化 网关URL和API Key
  useEffect(() => {
    localStorage.setItem('mg_gateway_url', gatewayUrl);
  }, [gatewayUrl]);
  useEffect(() => {
  ...
  const refreshModels = useCallback(async () => {
    ...
    try {
      ...
      setModelList(list);
      // 如果当前选中的模型不在新列表里，切到第一个
      if (!list.some(m => m.id === model)) {
        setModel(list[0]?.id || model);
      }
    } catch (e) {
  ...
  // 初始化 + 网关地址变化时自动拉模型
  useEffect(() => {
    if (gatewayUrl) refreshModels();
  }, [gatewayUrl]);

  // 持久化世界书
  useEffect(() => {
    saveWorldBook(worldBookEntries);
  }, [worldBookEntries]);

  // 持久化MCP配置
  useEffect(() => {
    localStorage.setItem('mg_mcp_servers', JSON.stringify(mcpServers));
  }, [mcpServers]);
  ...
  // === 文件处理 ===
  const onFileSelect = useCallback((e) => handleFileSelect(e, setPendingImages, setInput), []);
  const removePendingImage = useCallback((idx) => setPendingImages(prev => prev.filter((_, i) => i !== idx)), []);

  // 【功能2】编辑消息 — useCallback稳定引用，避免Bubble.memo失效
